  ...
  (function () {
    function t_events__initEvent() {
      ...
      function linkClickCreateEvent(e) {
        ...
        if (e.target && targetEl) {
          ...
          if ((Tilda.sendEventToStatistics(virtPage, virtTitle), "http" === url.substring(0, 4) && -1 === url.indexOf(".typeform.com/")))
            return (
              window.setTimeout(
                function () {
    ...
    (Tilda.sendEcommerceEvent = function (type, productsArr) {
      ...
      (Tilda.sendEventToStatistics = function (virtPage, virtTitle, referer, price) {
            ...
            if (isVirtPage)
              if (referer && referer.event)
                if ("purchase" === referer.event) {
                  ...
                  gtag("event", "purchase", { transaction_id: referer.ecommerce.purchase.actionField.id, value: parseFloat(price).toFixed(2), currency: currencyCode, shipping: delivery, items: referer.ecommerce.purchase.products });
            ...
            if (referer && referer.event) {
              try {
                if ((window.Tilda.isLoadGAEcommerce || ((window.Tilda.isLoadGAEcommerce = !0), ga("require", "ec")), ga("set", "currencyCode", currencyCode), "purchase" === referer.event)) {
                  for (var product, iProduct = referer.ecommerce.purchase.products.length, i = 0; i < iProduct; i++) (product = referer.ecommerce.purchase.products[i]), ga("ec:addProduct", { id: product.id || i, name: product.name, price: parseFloat(product.price).toFixed(2), quantity: product.quantity });
                  ga("ec:setAction", "purchase", { id: referer.ecommerce.purchase.actionField.id, revenue: parseFloat(price).toFixed(2) });
                } else if ("addToCart" === referer.event && referer.ecommerce.add) {
                  for (var product, iProduct = referer.ecommerce.add.products.length, i = 0; i < iProduct; i++) (product = referer.ecommerce.add.products[i]), ga("ec:addProduct", { id: product.id || i, name: product.name, price: parseFloat(product.price).toFixed(2), quantity: product.quantity });
                  ga("ec:setAction", "add");
                } else if ("removeFromCart" === referer.event && referer.ecommerce.remove) {
                  for (var product, iProduct = referer.ecommerce.remove.products.length, i = 0; i < iProduct; i++) (product = referer.ecommerce.remove.products[i]), ga("ec:addProduct", { id: product.id || i, name: product.name, price: parseFloat(product.price).toFixed(2), quantity: product.quantity });
                  ga("ec:setAction", "remove");
                } else if ("viewProduct" === referer.event && referer.ecommerce.detail) {
                  for (var product, iProduct = referer.ecommerce.detail.products.length, i = 0; i < iProduct; i++) (product = referer.ecommerce.detail.products[i]), ga("ec:addProduct", { id: product.id || i, name: product.name, price: parseFloat(product.price).toFixed(2), quantity: product.quantity });
                  ga("ec:setAction", "detail");
                }
              } catch (e) {}
              ga("send", { hitType: "pageview", page: virtPage, title: virtTitle, params: referer });
            } else ga("send", { hitType: "pageview", page: virtPage, title: virtTitle });
          else ga("send", { hitType: "event", eventCategory: "tilda", eventAction: virtPage, eventLabel: virtTitle, eventValue: price });
        ...
        if ((window.mainMetrika > "" && window[window.mainMetrika] && (isVirtPage ? (price > 0 ? window[window.mainMetrika].hit(virtPage, { title: virtTitle, order_price: price, params: referer }) : window[window.mainMetrika].hit(virtPage, { title: virtTitle })) : price > 0 ? window[window.mainMetrika].reachGoal(virtPage, { title: virtTitle, params: referer, order_price: price }) : window[window.mainMetrika].reachGoal(virtPage, { title: virtTitle, referer: referer })), void 0 !== window.fbq && !1 === noFbSendEvent))
          try {
            if (isVirtPage)
              if (-1 === virtPage.indexOf("tilda/") || (-1 === virtPage.indexOf("/payment/") && -1 === virtPage.indexOf("/submitted/")))
                if (referer && referer.event && price > 0)
                  if ("addToCart" === referer.event && referer.ecommerce.add) {
                    ...
                    window.fbq("track", "ViewContent", { content_ids: content_ids, content_type: "product", value: price, currency: currencyCode });
              ...
              if (referer && referer.event)
                if (((eventParams = { products: [], currency_code: "", total_price: 0 }), "purchase" === referer.event && referer.ecommerce.purchase))
                  if (price > 0 && priceListID > 0) {
                    ...
                    for (var iProduct = referer.ecommerce.purchase.products.length, content_ids = [], i = 0; i < iProduct; i++) (product = referer.ecommerce.purchase.products[i]), eventParams.products.push({ id: product.id || product.uid || product.name, price: product.price ? product.price : 0 }), (eventParams.total_price = price);
                  ...
                  if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
                    for (var iProduct = referer.ecommerce.add.products.length, content_ids = [], i = 0; i < iProduct; i++) (product = referer.ecommerce.add.products[i]), eventParams.products.push({ id: product.id || product.uid || product.name, price: product.price ? product.price : 0 }), (eventParams.total_price = price);
                  ...
                  if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
                    for (var iProduct = referer.ecommerce.detail.products.length, content_ids = [], i = 0; i < iProduct; i++) (product = referer.ecommerce.detail.products[i]), eventParams.products.push({ id: product.id || product.uid || product.name, price: product.price ? product.price : 0 }), (eventParams.total_price = price);
                  ...
                  if (price > 0 && priceListID > 0) {
                    var product;
                    eventParams.currency_code = currencyCode;
                    for (var iProduct = referer.ecommerce.remove.products.length, content_ids = [], i = 0; i < iProduct; i++) (product = referer.ecommerce.remove.products[i]), eventParams.products.push({ id: product.id || product.uid || product.name, price: product.price ? product.price : 0 }), (eventParams.total_price = price);
        ...
        if (window.mainMailruId > "0") {
          ...
          isVirtPage ? (price > 0 ? _tmr.push({ id: "" + window.mainMailruId, type: "pageView", url: virtPage, value: price, start: new Date().getTime() }) : _tmr.push({ id: "" + window.mainMailruId, type: "pageView", url: virtPage, start: new Date().getTime() })) : price > 0 ? _tmr.push({ id: "" + window.mainMailruId, type: "reachGoal", goal: virtPage, value: price }) : _tmr.push({ id: "" + window.mainMailruId, type: "reachGoal", goal: virtPage });
        }

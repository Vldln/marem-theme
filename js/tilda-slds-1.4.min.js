...
function t_slds_ActiveCaption(rec, position, totalSlides) {
  ...
  if (sliderRec) {
    ...
    Array.prototype.forEach.call(captions, function (caption) {
      ...
      0 == position ? (captionActive = sliderRec.querySelector('.t-slds__caption[data-slide-caption="' + totalSlides + '"]')) : position == totalSlides + 1 && (captionActive = sliderRec.querySelector('.t-slds__caption[data-slide-caption="1"]')),
...
function t_slds_ActiveBullet(rec, position, totalSlides, sliderOptions) {
  ...
  if (sliderRec) {
    ...
    if (
      ...
      (sliderOptions && sliderOptions.thumbsbulletGallery && position >= maxThumbsCount && position != totalSlides + 1) || (totalSlides >= maxThumbsCount && 0 == position) ? (bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="' + maxThumbsCount + '"]')) : 0 == position ? (bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="' + totalSlides + '"]')) : position == totalSlides + 1 && (bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="1"]')),
      bulletActive)
    ) {
...
function t_slds_ActiveSlide(rec, position, totalSlides) {
  ...
  if (sliderRec) {
    ...
    if (
      ...
      sliderRec.querySelector('.t-slds__item[data-slide-index="' + totalSlides + '"]').classList.add("t-slds__item_active"), sliderRec.setAttribute("aria-hidden", "false");
  ...
  if (sliderRec) {
    ...
    if (sliderMain) {
      ...
      if (sliderWrapper) {
        ...
        delete Hammer.defaults.cssProps.userSelect, (hammer = new Hammer(sliderMain, { domEvents: !0, inputClass: Hammer.TouchInput, recognizers: [[Hammer.Pan, { direction: Hammer.DIRECTION_HORIZONTAL }]] })), sliderWrapper.setAttribute("data-swiper-initialized", "true");
        ...
        window.removeEventListener("touchmove", scrollPrevent, { passive: !1 }),
          ...
          hammer.on("pan", function (event) {
            ...
            if ((sliderWrapper.setAttribute("data-slider-touch", "yes"), t_slds_scrollImages(rec, sliderWidth * position - distance), event.isFinal)) {
              if (event.velocityX > 0.4) "false" == sliderWrapper.getAttribute("data-slider-with-cycle") && 1 == position ? (position = 1) : position--, sliderWrapper.setAttribute("data-slider-pos", position), 0 == position && (cycle = "yes"), sliderWrapper.setAttribute("data-slider-cycle", cycle), t_slideMove(rec, !1, sliderOptions);

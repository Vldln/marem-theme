  ...
  (allElements = Array.prototype.filter.call(allElements, function (el) {
    ...
    Array.prototype.forEach.call(allElements, function (el) {
      if (!el.classList.contains("t975")) {
        ...
        ("fixed" === computedPosition || ("absolute" === computedPosition && isFullscreenWidth && !isFullscreenHeight)) && obj.fixedElements.push({ el: el, computedStyle: computedStyle });
      }
    });
}
...
function t_scrollBarWidthCompensator__cancel() {
  ...
  obj &&
    ...
    ((obj.isInited = !1),
    (obj.delay = 0),
    document.body.hasAttribute("data-tilda-initial-padding-right") ? ((document.body.style.paddingRight = document.body.getAttribute("data-tilda-initial-padding-right")), document.body.removeAttribute("data-tilda-initial-padding-right")) : document.body.style.removeProperty("padding-right"),
...
(window.browserLang = (window.navigator.userLanguage || window.navigator.language).toUpperCase().slice(0, 2)),
  ...
  t_onReady(function () {
    ...
    if (-1 !== userAgent.indexOf("Android") && (isFacebook || isInstagram || isYandex || isSamsung || isDuckDuckGo)) {
      ...
      textElement.parentNode.removeChild(textElement), factor < 0.98 && document.body.insertAdjacentHTML("beforeend", '<style>.t396 [data-elem-type="text"] .tn-atom {zoom: ' + 100 * factor + "%;}</style>");
    }
  ...
  t_onReady(function () {
    ...
    if (!window.isMobile && allRecords && "yes" !== allRecords.getAttribute("data-blocks-animationoff") && !1 === window.isSearchBot) {
      ...
      for (
        var recBlocksNot = Array.prototype.slice.call(recBlocks).filter(function (el) {
        ...
        var rec,
          recTop = (rec = recBlocksNot[i]).getBoundingClientRect().top + window.pageYOffset,
  ...
  (function () {
    ...
    (window.isMobile || window.parent.isPagePreview) &&
      ...
      window.innerWidth < 480 || (window.isMobile && document.documentElement.clientWidth < 480) || (isInstagram && screen.width < 480)
        ? (t_onReady(function () {
        ...
        : (window.innerWidth < 900 || (window.isMobile && document.documentElement.clientWidth < 900) || (isInstagram && screen.width < 900)) &&
          t_onReady(function () {
            for (var customStyleElements = document.querySelectorAll('[data-customstyle="yes"]'), fieldElements = document.querySelectorAll("[field] span, [field] strong, [field] em, [field] a"), i = 0; i < customStyleElements.length; i++) {
              var element = customStyleElements[i];
  ...
  (function (window, Math) {
    window.onerror = function (message, filename, lineno, colno, error) {
      "object" != typeof window.t_jserrors && (window.t_jserrors = []), window.t_jserrors.push({ message: message, filename: filename, lineno: lineno, colno: colno, error: error });
    };
  })(window, Math),
  t_onReady(function () {
    document.body.addEventListener("popupShowed", t_scrollBarWidthCompensator__init),
      document.body.addEventListener("popupHidden", function () {
        var obj = window.scrollBarWidthCompensator;
        obj &&
          (obj.cancelTimeout && (window.clearTimeout(obj.cancelTimeout), (obj.cancelTimeout = null)),
